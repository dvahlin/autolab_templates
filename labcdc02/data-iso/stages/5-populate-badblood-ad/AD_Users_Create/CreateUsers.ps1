...
do {
    ...
    $surname = get-content($scriptpath + '\Names\efternamn.txt')|get-random
$genderpreference = 0,1|get-random
if ($genderpreference -eq 0){$givenname = get-content($scriptpath + '\Names\tjejer.txt')|get-random}else{$givenname = get-content($scriptpath + '\Names\killar.txt')|get-random}
$name = $givenname+" "+$surname
$accountname = [char[]]$givenname[0..2] + [char[]]$surname[0..2] -join ''
}
...
#$pwd = New-SWRandomPassword -MinPasswordLength 5 -MaxPasswordLength 15
$pwd = get-content($scriptpath + '\Passwords\test.txt')|get-random
#======================================================================
#if RA account - create account disabled

    try{new-aduser -server $setdc  -Description $Description -DisplayName $name -name $name -SamAccountName $accountname.ToLower() -Surname $name -Enabled $true -Path $ouLocation -AccountPassword (ConvertTo-SecureString ($pwd) -AsPlainText -force)}
            catch {$pwd = New-SWRandomPassword -MinPasswordLength 8 -MaxPasswordLength 12
